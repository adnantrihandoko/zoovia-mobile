...
class AntrianRemoteDatasource {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          return antrianList.map((json) => AntrianModel.fromJson(json)).toList();
        } else {
          throw ServerFailure(data['message'] ?? 'Gagal mengambil data antrian');
        }
      } else {
  ...
  Future<List<AntrianModel>> getAntriansByUserId(int userId, String token, {String? status}) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          ...
          throw ServerFailure(data['message'] ?? 'Gagal mengambil data antrian user');
        }
      } else {
        throw ServerFailure('Gagal mengambil data: ${response.statusCode}');
      }
  ...
  Future<AntrianModel> createAntrian(Map<String, dynamic> data, String token) async {
    try {
      ...
      if (response.statusCode == 201) {
        ...
        if (responseData['success'] == true) {
          ...
          throw ServerFailure(responseData['message'] ?? 'Gagal membuat antrian');
        }
      } else {
  ...
  // Mengupdate antrian
  Future<AntrianModel> updateAntrian(int id, Map<String, dynamic> data, String token) async {
    try {
      final response = await _dio.post(
        '/antrian/$id',
        data: data,
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          ...
          throw ServerFailure(responseData['message'] ?? 'Gagal mengupdate antrian');
        }
      } else {
  ...
  Future<AntrianModel> updateAntrianStatus(int id, String status, String token) async {
    try {
      final response = await _dio.post(
        '/antrian/$id/status',
        data: {'status': status},
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['success'] == true) {
          ...
          throw ServerFailure(responseData['message'] ?? 'Gagal mengupdate status antrian');
        }
      } else {
        throw ServerFailure('Gagal mengupdate status antrian: ${response.statusCode}');
      }
    } on DioException catch (e) {
      throw NetworkFailure('Terjadi kesalahan jaringan: ${e.message}');
    } catch (e) {
